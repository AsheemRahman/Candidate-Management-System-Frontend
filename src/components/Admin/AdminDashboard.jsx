...
const AdminDashboard = () => {
    ...
    const fetchCandidates = async () => {
        try {
            const { data } = await axios.get("http://localhost:8080/api/admin/candidates", { withCredentials: true });
            setCandidates(data);
        } catch (error) {
            console.error(error);
        }
    ...
    const handleCreateCandidate = async (e) => {
        e.preventDefault();
        try {
            await axios.post("http://localhost:8080/api/admin/candidate/create",
    ...
    const handleDelete = async (id) => {
        try {
            const response = await axios.delete(`http://localhost:8080/api/admin/candidate/delete/${id}`, { headers: { Authorization: localStorage.getItem("token") } });
    ...
    const handleLogout = async () => {
        try {
            const response = await fetch('http://localhost:8080/api/admin/signout', {
                method: 'POST',
                headers: {
